...
const structuredDataProducts = items.map((item) => {
  const absoluteImg = `https://rcrsjaiswal.in${item.imgSrc}`;
  const productUrl = `https://rcrsjaiswal.in${item.link}`;
  ...
  // Only include offers when a price exists to avoid invalid schema
  if (item.price) {
    base.offers = {
      "@type": "Offer",
      priceCurrency: "INR",
